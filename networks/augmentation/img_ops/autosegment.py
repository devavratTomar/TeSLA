"""
Differentiable Torch Implementation of AutoAugment Image Functions for segmentation
"""
...
def ShearX(x, v):
    ...
    affine = identity + shear_x

    out =  apply_affine(x, affine)
    
    return out, affine.detach()
...
def AutoContrast(x, v):
    # autocontrast crashes with shearX is backward graph is computed. Maybe a bug in the official implementation while resampling in grid_affine.
    out = tv.transforms.functional.autocontrast(x.detach()) #+ v - v.detach()
    v = v.reshape(-1, 1, 1, 1)

    identity = torch.tensor([1, 0, 0, 0, 1, 0], dtype=torch.float32, device=x.device).reshape(-1, 2, 3)

    return out + x - x.detach() + v - v.detach(), identity
...
def Invert(x, v):
    ...
    return out + v - v.detach(), identity
...
def Solarize(x, v):
    ...
    return x_out + v - v.detach() + x - x.detach(), identity


def Posterize(x, v):
    x_uint = (255.0 * x).to(torch.uint8)
    # shift = (8 - v).to(torch.int64)
    ...
    return x_out + x - x.detach() + v - v.detach(), identity
...
def Contrast(x, v):
    ...
    return blend(degenerate, x, v), identity
