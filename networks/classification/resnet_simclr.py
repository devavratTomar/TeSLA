...
class AdaptSupCESimCLRResNet(nn.Module):
    def __init__(self, name='resnet50', num_classes=10):
        super(AdaptSupCESimCLRResNet, self).__init__()
        self.encoder = get_resnet(depth=int(name.replace("_simclrv2", "").replace("resnet", "")))[0]
        self.fc = nn.Linear(self.encoder.fc.in_features, num_classes)
